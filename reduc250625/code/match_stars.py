...
def main():
    ...
    for fitnm in imgs:
        try:
            ...
            print(f'Error reading FITS header or WCS for {fitnm}: {e}')
            continue

        # 自动选取目标附近的亮星
        pos_csv = os.path.splitext(os.path.basename(fitnm))[0] + '_pos.csv'
        ...
        # 绘图标注
        plot_grb_and_refs(fitnm, grb_x, grb_y, ref_stars)
        if len(ref_stars) < 3:
            print(f'Warning: {pos_csv} 附近亮参考星太少，直接用WCS反投影')
            results.append({'frame': fitnm, 'xcent': grb_x, 'ycent': grb_y, 'method': 'wcs'})
            continue
        # 用这些星的天球坐标和像素坐标做仿射变换
        ...
        results.append({'frame': fitnm, 'xcent': grb_xy[0], 'ycent': grb_xy[1], 'ra': grb_ra, 'dec': grb_dec, 'method': 'affine'})

    # 4. 保存结果
    df_out = pd.DataFrame(results)
    df_out.to_csv('grb_xyposs.csv', index=False)
    print('已保存每一帧的GRB像素坐标到 grb_xyposs.csv')
